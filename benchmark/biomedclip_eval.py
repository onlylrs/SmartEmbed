#!/usr/bin/env python3
"""
BiomedCLIP Zero-shot Cross-modal Retrieval Evaluation

Usage:
    python biomedclip_eval.py
    python biomedclip_eval.py --data_jsonl /path/to/eval.jsonl --batch_size 64
"""
...
def evaluate_biomedclip(jsonl_path: str, batch_size: int, device: str) -> Dict[str, Dict[str, float]]:
    """Evaluate BiomedCLIP on cross-modal retrieval task"""
    ...
    for i in tqdm(range(0, len(images), batch_size), desc="Image batches"):
        ...
        if batch_imgs:
            ...
            features, _, _ = model(batch_tensor, None)
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="Text batches"):
        ...
        batch_tokens = tokenizer(batch_texts, context_length=256).to(device)
        _, features, _ = model(None, batch_tokens)
        features = features / features.norm(dim=-1, keepdim=True)
...
def main():
    parser = argparse.ArgumentParser(description="BiomedCLIP cross-modal retrieval evaluation")
    ...
    results = evaluate_biomedclip(args.data_jsonl, args.batch_size, args.device)
    
    # Print results
    print("\nBiomedCLIP Zero-shot Retrieval Results:")
    ...
    out_file = Path(args.save_dir) / f"biomedclip_{ts}.json"
    json.dump({"model": "BiomedCLIP", "data": args.data_jsonl, "results": results}, out_file.open("w"), indent=2)
